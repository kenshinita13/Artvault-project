...
// Get current user ID, role, profile picture and name for navigation/actions
...
// Get artist details
$artist_res = $conn->query("SELECT * FROM users WHERE id = $profile_id");
if ($artist_res->num_rows == 0) {
    header("Location: profiles.php");
    exit();
}

$profile_user = $artist_res->fetch_assoc();
...
<head>
    ...
    <title><?= htmlspecialchars($profile_name) ?> (@<?= htmlspecialchars($profile_username) ?>) Studio - ArtVault</title>
    ...
    <main class="studio-container">
        ...
        <div class="studio-header-card">
            ...
            <div class="studio-meta">
                ...
                <div class="studio-username">@<?= htmlspecialchars($profile_username) ?></div>
                <div class="studio-badges">
                    <span class="badge <?= $profile_role === 'admin' ? 'admin' : '' ?>">
                        <?= $profile_role === 'admin' ? '🛡️ Administrator' : '🎨 Artist' ?>
                    </span>
                    <?php
                    $count_res = $conn->query("SELECT COUNT(*) as total FROM images WHERE user_id = $profile_id");
                    $count_row = $count_res->fetch_assoc();
                    $num_creations = $count_row ? (int)$count_row['total'] : 0;
                    ?>
                    <span class="badge">
                        ✨ <?= $num_creations ?> Creations
                    </span>
                </div>
            </div>
        </div>

        <h2 style="font-size: 24px; font-weight: 700; margin-bottom: 25px; display: flex; align-items: center; gap: 10px;">
            🖼️ Studio Showcase
        </h2>
        ...
        <div class="gallery-grid">
            ...
            $query = "SELECT images.*, users.name as artist_name, users.username as artist_username 
                      ...
                      WHERE images.user_id = $profile_id 
            ...
            ?>
                <div style="grid-column: 1 / -1; text-align: center; padding: 80px 20px; color: var(--text-secondary);">
                    ...
                    <h4 style="margin-top: 15px;">No Artworks Uploaded Yet</h4>
                    <p style="margin-top: 5px;">This artist hasn't published any creations to the showcase gallery.</p>
                </div>
            <?php
            else:
                ...
                <div class="art-card" onclick="openLightbox(
                    ...
                    <div class="art-details">
                        ...
                        <div class="art-actions" onclick="event.stopPropagation();">
                            ...
                            <?php if ($can_delete): ?>
                                <a href="delete.php?id=<?= $row['id'] ?>&redirect=profile&profile_id=<?= $profile_id ?>" onclick="return confirm('Delete this image?')" class="btn btn-danger">
                                    🗑️ Delete
                                </a>
                            <?php endif; ?>
                        </div>
